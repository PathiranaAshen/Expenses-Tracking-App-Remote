                ...
                HStack(spacing: 0) {
                    ...
                    VStack {
                        ...
                        VStack {
                            DashboardCard(title: "Income", value: viewModel.dashboardData.income, color: .green)
                            DashboardCard(title: "Expenses", value: viewModel.dashboardData.expenses, color: .red)
                            DashboardCard(title: "Budget", value: viewModel.dashboardData.budget, color: .yellow)
                            DashboardCard(title: "Savings", value: viewModel.dashboardData.savings, color: .blue)
                        }
                        .padding()

                        if viewModel.dashboardData.income != 0.0 || viewModel.dashboardData.expenses != 0.0 {
                                                    Text("Savings Percentage: \(viewModel.dashboardData.savingsPercentage >= 0 ? viewModel.dashboardData.savingsPercentage : 0, specifier: "%.2f")%")
                                                        .font(.title2)
                                                        .bold()
                                                        .padding(.bottom, 10)
                                                } else {
                                                    Text("Savings Percentage: 0%")
                                                        .font(.title2)
                                                        .bold()
                                                        .padding(.bottom, 10)
                                                }
